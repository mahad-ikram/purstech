      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4">
            Tips, Guides &amp; <span className="bg-gradient-to-r from-[#6C3AFF] to-[#00D4FF] bg-clip-text text-transparent">Tool News</span>
          </h1>
          ...
          <Link href={`/blog/${featured.slug}`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 md:p-12">
              ...
              <div className="hidden md:flex items-center justify-center">
                <div className="w-full h-64 bg-gradient-to-br from-[#6C3AFF]/30 to-[#00D4FF]/20 rounded-2xl flex items-center justify-center text-7xl">📖</div>
              </div>
            </div>
          </Link>
        )}
              ...
              {categories.map(cat => (
                <button key={cat} onClick={() => setCategory(cat)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
                    category === cat ? "bg-[#6C3AFF] text-white shadow-lg" : "bg-[#0A0A14] border border-white/5 text-gray-400 hover:text-white hover:border-white/10"
                  }`}>
                  {cat}
                </button>
              ))}
            ...
            {filteredPosts.map((post) => (
              <Link key={post.slug} href={`/blog/${post.slug}`}
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-xs bg-[#6C3AFF]/10 text-[#6C3AFF] border border-[#6C3AFF]/20 px-2.5 py-1 rounded-full font-semibold">{post.category}</span>
                  <span className="text-xs text-gray-600">{post.readTime}</span>
                </div>
                <h2 className="font-extrabold text-white text-base leading-snug group-hover:text-[#6C3AFF] transition-colors line-clamp-2">{post.title}</h2>
                <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 flex-1">{post.excerpt}</p>
                <div className="flex items-center justify-between pt-3 border-t border-white/5">
                  <time dateTime={post.publishedISO} className="text-xs text-gray-600">{post.publishedAt}</time>
                  <span className="text-xs text-[#6C3AFF] font-semibold group-hover:text-[#00D4FF] transition-colors">Read article →</span>
                </div>
              </Link>
            ))}
          </div>
