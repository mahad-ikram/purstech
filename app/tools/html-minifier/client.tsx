...
function minifyHTML(
  html: string,
  opts: { removeComments: boolean; collapseWhitespace: boolean; removeOptionalTags: boolean }
): string {
  let result = html;
  if (opts.removeComments)     result = result.replace(/<!--[\s\S]*?-->/g, "");
  if (opts.collapseWhitespace) result = result.replace(/\s+/g, " ").replace(/>\s+</g, "><").trim();
  if (opts.removeOptionalTags) result = result.replace(
    /<\/?(html|head|body|li|dt|dd|p|rt|rp|optgroup|option|colgroup|thead|tbody|tfoot|tr|td|th)\b[^>]*>/gi,
...
export default function HTMLMinifierClient() {
  // ✅ Track usage
  useTrackTool("html-minifier", "dev");
  ...
  // ✅ UI Enhancement 1: Download as .html file
  const handleDownload = () => {
    if (!output) return;
    const blob = new Blob([output], { type: "text/html" });
    const a    = Object.assign(document.createElement("a"), {
      href:     URL.createObjectURL(blob),
      download: "index.min.html",
    });
    a.click();
  ...
  const savings       = originalSize > 0 && minifiedSize > 0
    ? Math.round((1 - minifiedSize / originalSize) * 100) : 0;
  // ✅ UI Enhancement 2: char count alongside bytes
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <div key={opt.key} className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => setOpts(o => ({ ...o, [opt.key]: !o[opt.key as keyof typeof o] }))}
                      ...
                      role="switch"
                      aria-checked={opts[opt.key as keyof typeof opts]}>
            ...
            {output && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  {/* ✅ UI Enhancement 2: char count + bytes */}
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                    <span className="text-gray-500">
                      Original: <span className="text-white font-bold">{originalChars.toLocaleString()} chars</span>
                      <span className="text-gray-600 ml-1">({originalSize} bytes)</span>
                    </span>
                    <span className="text-gray-500">
                      Minified: <span className="text-white font-bold">{minifiedChars.toLocaleString()} chars</span>
                      <span className="text-gray-600 ml-1">({minifiedSize} bytes)</span>
                    </span>
                    {savings > 0 && <span className="text-green-400 font-bold">↓ {savings}% smaller</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    {/* ✅ UI Enhancement 1: Download .html */}
            ...
            <div className="bg-[#13131F] border border-white/5 rounded-2xl p-5">
              ...
              <div className="space-y-2 text-xs text-gray-500">
                {[
                  ...
                  "Optional closing tags (if enabled)",
                ].map(r => (
        ...
        <section className="mt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { step:"1", title:"Set Your Options",  desc:"Choose which minification steps to apply. Remove Comments and Collapse Whitespace are safe for all HTML. Remove Optional Tags is more aggressive and should be tested." },
              { step:"2", title:"Paste Your HTML",   desc:"Copy your HTML from your editor and paste it in. Click Load Sample to see a typical before/after comparison." },
              { step:"3", title:"Minify & Download", desc:"Click Minify HTML. Size savings shown immediately. Copy to clipboard or download as index.min.html for your deployment." },
            ].map(s => (
