...
const SCHEMA = {
  ...
  description: "Free online HTML to Markdown converter with GFM table support, code block handling and clipboard paste.",
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Iframes, scripts, style tags and other non-content HTML elements have no Markdown equivalent and are stripped out during conversion. This is typically the desired behaviour — you want the text content, not scripts or tracking code. If you need to embed external content in Markdown, raw HTML blocks are supported in most Markdown processors: you can insert raw <iframe> HTML directly in a Markdown file and it will render in most environments.",
  },
  {
    q: "Can I convert an entire web page by pasting its HTML?",
    a: "Yes — paste the full HTML source (including head, body, scripts, nav, footer etc.) and our converter will extract the meaningful text content while stripping structural HTML, scripts, styles and invisible elements. For large pages, the converter focuses on the main content tags. For best results when extracting a specific article, copy just the article's HTML rather than the entire page source.",
  },
];
...
interface ConvertOptions {
  ...
  keepImages:  boolean;
  setext:      boolean; // underline-style headings
  bulletChar:  "-" | "*" | "+";
}
...
function nodeToMd(node: Node, opts: ConvertOptions, depth = 0): string {
  ...
  switch (tag) {
    ...
    case "pre": {
      ...
      const langAttr = codeEl?.className.match(/language-(\w+)/)?.[1] ?? "";
      const content  = (codeEl?.textContent ?? el.textContent ?? "").trim();
      if (opts.codeBlocks) return `\`\`\`${langAttr}\n${content}\n\`\`\`\n\n`;
    ...
    case "span":
    default:
      return inner().trim() ? inner() + (["div","section","article","main"].includes(tag) ? "\n\n" : "") : "";
  }
}
...
function htmlToMarkdown(html: string, opts: ConvertOptions): string {
  ...
  const body   = doc.body;
  if (!body) return "";
  const md = Array.from(body.childNodes).map(c => nodeToMd(c, opts)).join("").trim();
    ...
    <div className="min-h-screen bg-[#0A0A14] text-white font-sans">
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 max-w-2xl">Convert HTML to clean, readable Markdown. Preserves headings, bold, italic, links, images, code blocks, tables and lists. Outputs GitHub Flavored Markdown with full customisation options.</p>
        </div>
        ...
        <div className="mt-4 flex gap-4 text-xs text-gray-500 flex-wrap">
          ...
          <span>Reduction: {input.length > 0 ? ((1 - output.length / input.length) * 100).toFixed(0) : 0}%</span>
