...
const SCHEMA = {
  ...
  description: "Free online Markdown editor with live split-pane preview, full toolbar, GFM tables, task lists and HTML/MD export.",
...
function renderMarkdown(md: string): string {
  ...
  // Escape HTML (in non-code areas) — simple approach
...
<style>
  ...
  h1,h2,h3,h4,h5,h6 { margin-top: 1.5em; } pre { background: #f5f5f5; padding: 1rem; border-radius: 8px; overflow-x: auto; }
  ...
  th { background: #f0f0f0; } blockquote { border-left: 4px solid #6C3AFF; margin: 0; padding: 0.5rem 1rem; color: #555; }
...
${html}
  ...
  return (
    <div className="min-h-screen bg-[#0A0A14] text-white font-sans">
      ...
      <main className={`${fullscreen ? "" : "max-w-7xl mx-auto px-4 py-10"}`}>
        {!fullscreen && (
          <>
        ...
        {/* Editor container */}
        <div className={`bg-[#13131F] border border-white/5 rounded-2xl overflow-hidden ${containerClass}`}>
          {/* Toolbar */}
          ...
          {/* Editor / Preview */}
          <div className={`flex ${fullscreen ? "flex-1 min-h-0" : "h-[600px]"} overflow-hidden`}>
            {(layout === "editor" || layout === "split") && (
              <textarea
                ...
                spellCheck={false}
                className={`${layout === "split" ? "w-1/2" : "w-full"} h-full px-5 py-4 bg-[#0A0A14] text-white text-sm font-mono resize-none focus:outline-none leading-relaxed border-r border-white/5`}
              />
            )}
            {(layout === "preview" || layout === "split") && (
              <div className={`${layout === "split" ? "w-1/2" : "w-full"} h-full overflow-y-auto`}>
                <div
                  className={`min-h-full p-6 prose max-w-none ${darkPreview ? "bg-gray-900 text-gray-100" : "bg-white text-gray-900"}`}
            ...
            <div className="mt-10 bg-[#13131F] border border-white/5 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                {[
                  ...
                  { step:"2", title:"Watch the live preview", desc:"The right pane renders your Markdown in real time. Switch between Split, Editor-only or Preview-only layouts using the view buttons." },
                  { step:"3", title:"Toggle dark preview mode", desc:"Click the sun/moon button to switch the preview between light and dark mode — useful for checking how your content looks in different environments." },
                  { step:"4", title:"Export in your format", desc:"Download your document as a .md file for GitHub, Git repos or Markdown platforms, or export as a complete HTML file ready to open in any browser." },
                ].map(s => (
