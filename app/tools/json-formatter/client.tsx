  ...
  return json
    ...
    .replace(
      ...
      (match) => {
        let cls = "text-[#00D4FF]";
        if (/^"/.test(match)) {
          cls = /:$/.test(match) ? "text-[#6C3AFF]" : "text-green-400";
...
export default function JSONFormatterClient() {
  ...
  const handleFormat = () => {
    ...
    try {
      ...
      setError(""); setMode("format");
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      setError(`❌ Invalid JSON: ${msg}`); setOutput("");
    }
  };
  ...
  const handleDownload = () => {
    ...
    const ext  = mode === "minify" ? "min.json" : "formatted.json";
  ...
  const savings    = inputSize > 0 && outputSize > 0
    ? Math.round(((inputSize - outputSize) / inputSize) * 100) : 0;
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0A0A14] text-white font-sans flex flex-col overflow-x-hidden">
      ...
      {/* ✅ CRITICAL QA FIX: flex-grow w-full ensures footer behaves */}
      <main className="max-w-7xl mx-auto px-4 py-10 flex-grow w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* ── LEFT: Editor ── */}
          {/* ✅ CRITICAL QA FIX: min-w-0 prevents flex blowout */}
          <div className="lg:col-span-2 flex flex-col gap-4 min-w-0">
                ...
                {[2, 4].map(n => (
                  <button key={n} onClick={() => setIndent(n)}
                    className={`w-8 h-7 rounded-lg text-xs font-bold transition-all ${
                      indent === n ? "bg-[#6C3AFF] text-white" : "text-gray-400 hover:text-white"
                    }`}>{n}</button>
                ))}
              </div>
            ...
            {error && (
              <div className="bg-red-500/10 border border-red-500/30 rounded-xl px-5 py-3 text-red-400 text-sm font-mono break-words">
                {error}
              </div>
            )}
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center justify-between mb-2 gap-3">
                  <label className="text-xs text-gray-600 font-medium uppercase tracking-wider">
                    {mode === "format" ? "Formatted JSON" : mode === "minify" ? "Minified JSON" : "Result"}
                  </label>
                  <div className="flex items-center gap-2">
                    {mode === "minify" && savings > 0 && (
                      <span className="text-xs text-green-400 font-semibold">{savings}% smaller</span>
                    )}
                    {mode !== "validate" && (
                ...
                <div className="relative bg-[#13131F] border border-white/5 rounded-2xl overflow-hidden min-w-0">
                  <pre className="p-5 text-sm font-mono leading-relaxed overflow-auto max-h-96 text-gray-300 whitespace-pre-wrap break-all"
            ...
            {(input || output) && (
              <div className="flex flex-wrap gap-4 text-xs text-gray-600">
                {input && (
                  <span>Input size: <span className="text-gray-400 font-semibold">
                    {inputSize > 1024 ? `${(inputSize/1024).toFixed(1)} KB` : `${inputSize} bytes`}
                  </span></span>
                )}
                {output && mode !== "validate" && (
                  <span>Output size: <span className="text-gray-400 font-semibold">
                    {outputSize > 1024 ? `${(outputSize/1024).toFixed(1)} KB` : `${outputSize} bytes`}
                  </span></span>
                )}
              </div>
            )}
              ...
              <div className="space-y-3 text-xs text-gray-500">
                {[
                  ...
                  <div key={err} className="flex items-start gap-2">
                    <span className="text-red-500 mt-0.5">✕</span><span>{err}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              { step:"3", title:"Copy or Download", desc:"Your formatted JSON appears with colour syntax highlighting. Click Copy to use it anywhere, or ⬇ .json to save it as a file." },
            ].map(s => (
        ...
        {/* ✅ CRITICAL QA FIX: Replaced React state buttons with AdSense-compliant <details> tags */}
        <section className="mt-16">
      ...
      <footer className="border-t border-white/5 mt-auto py-8 text-center bg-[#0A0A14]">
        ...
        <p className="text-gray-700 text-xs mt-2">© 2026 PursTech. All rights reserved.</p>
      </footer>
    </div>
  );
}
