...
export default function LoanCalculatorClient() {
  // ✅ Track usage
  useTrackTool("loan-calculator", "finance");
  ...
  const pctInterest = result
    ? (result.totalInterest / (result.totalInterest + +amount)) * 100 : 0;
  ...
  // ✅ UI Enhancement 2: Download amortization as CSV
  const downloadAmortCSV = () => {
    if (!result) return;
    ...
    Object.assign(document.createElement("a"), {
      ...
      download: "loan-amortization.csv",
    }).click();
  };
  ...
  const InputField = ({
    ...
    <div className="min-w-0">
      ...
      <div className="flex items-center">
        {prefix && (
          <span className="px-3 py-2.5 bg-[#0A0A14] border border-r-0 border-white/10 rounded-l-xl text-gray-400 text-sm">
            {prefix}
          </span>
        )}
        <input value={value} onChange={e => onChange(e.target.value)} type="number" min="0"
          className={`flex-1 px-4 py-2.5 bg-[#0A0A14] border border-white/10 text-white text-sm focus:outline-none focus:border-[#6C3AFF]/60 transition-all min-w-0 ${prefix ? "" : "rounded-l-xl"} ${suffix ? "" : "rounded-r-xl"}`} />
        {suffix && (
          <span className="px-3 py-2.5 bg-[#0A0A14] border border-l-0 border-white/10 rounded-r-xl text-gray-400 text-sm">
            {suffix}
          </span>
        )}
      </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10 flex-grow w-full">
        ...
        <div className="mb-6 bg-yellow-500/5 border border-yellow-500/20 rounded-xl px-4 py-3 flex items-start gap-2">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            This calculator provides estimates for informational purposes only. Actual loan terms, rates and fees vary by lender and borrower profile. Consult a qualified financial advisor before making borrowing decisions.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 min-w-0">
          ...
          <div className="lg:col-span-2 space-y-4 min-w-0">
            <div className="bg-[#13131F] border border-white/5 rounded-2xl p-5 space-y-4 min-w-0">
            ...
            <div className="bg-[#13131F] border border-white/5 rounded-2xl p-5 space-y-3 min-w-0">
            ...
            <button onClick={() => setCompare(p => !p)}
              className={`w-full py-2.5 rounded-xl text-sm font-bold transition-all border ${
                compare ? "bg-[#6C3AFF] text-white border-transparent" : "bg-[#13131F] border-white/10 text-gray-400 hover:text-white"
              }`}>
              {compare ? "✓ Comparing Two Loans" : "⇔ Compare Two Loans"}
            </button>

            {compare && (
              <div className="bg-[#13131F] border border-[#6C3AFF]/20 rounded-2xl p-5 space-y-4 min-w-0">
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3 min-w-0">
                  {[
                    ...
                    <div key={s.label} className="bg-[#13131F] border border-white/5 rounded-2xl p-4 text-center min-w-0">
                      <div className={`font-extrabold ${s.big ? "text-2xl" : "text-base"} ${s.color} truncate`}>{s.value}</div>
                ...
                {/* Comparison table */}
                {compare && result2 && (
                  <div className="bg-[#13131F] border border-[#6C3AFF]/20 rounded-2xl p-5 overflow-hidden">
                    <h3 className="text-sm font-bold text-white mb-4">Loan A vs Loan B Comparison</h3>
                    {/* ✅ CRITICAL QA FIX: Isolated mobile-safe wrapper for the comparison table */}
                ...
                <div className="bg-[#13131F] border border-white/5 rounded-2xl p-5 overflow-hidden">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <button onClick={() => setShowAmort(p => !p)} className="flex items-center gap-2 flex-1 text-left min-w-0">
                      <span className="font-bold text-white text-sm truncate">
                        Full Amortization Schedule ({result.actualMonths} payments)
                      </span>
                      <span className={`text-[#6C3AFF] text-xl transition-transform flex-shrink-0 ${showAmort ? "rotate-45" : ""}`}>+</span>
                    </button>
                    {showAmort && (
                      <button onClick={downloadAmortCSV}
                        className="px-3 py-1.5 rounded-lg bg-[#0A0A14] border border-white/10 text-gray-400 hover:text-white text-xs font-bold transition-all flex-shrink-0">
                        ⬇ CSV
                      </button>
                    )}
                  </div>
                  ...
                  {showAmort && (
                    /* ✅ CRITICAL QA FIX: Isolated mobile-safe wrapper for the amortization table */
        ...
        <div className="mt-10 bg-[#13131F] border border-white/5 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              { step:"3", title:"Review amortization schedule",    desc:"Expand the full schedule to see every monthly payment broken into principal and interest. Download as CSV for spreadsheet analysis." },
              { step:"4", title:"Compare two loans",               desc:"Toggle comparison mode to enter a second loan and see a side-by-side breakdown of monthly payment, total interest and payoff date." },
            ].map(s => (
