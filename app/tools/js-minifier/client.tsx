...
export default function JSMinifierClient() {
  ...
  function download() {
    ...
    Object.assign(document.createElement("a"), {
      href:     URL.createObjectURL(new Blob([output], { type: "text/javascript" })),
      download: `script.${ext}`,
    }).click();
  }
    ...
    <div className="min-h-screen bg-[#0A0A14] text-white font-sans flex flex-col overflow-x-hidden">
      ...
      <main className="max-w-6xl mx-auto px-4 py-10 flex-grow w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {[
            { label:"Original",   value:fmtBytes(origBytes), sub:`${origChars.toLocaleString()} chars`, color:"text-white"     },
            { label:"Output",     value:fmtBytes(outBytes),  sub:`${outChars.toLocaleString()} chars`,  color:"text-[#6C3AFF]" },
            { label:"Saved",      value:`${savings}%`,       sub:`${gzipSavings}% with gzip`,           color:+savings > 0 ? "text-green-400" : "text-gray-500" },
            { label:"~Gzip Size", value:fmtBytes(gzipEst),   sub:"server delivery",                     color:"text-cyan-400"  },
          ].map(s => (
            <div key={s.label} className="bg-[#13131F] border border-white/5 rounded-xl p-3 text-center min-w-0">
          ...
          <div className="mt-5 bg-[#13131F] border border-white/5 rounded-2xl p-5">
            ...
            {showDiff && (
              <div className="mt-4 space-y-2">
                {minified.passes.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-wrap items-center gap-3 text-sm">
                      ...
                      <span className="flex-1 text-gray-300 break-words">{p}</span>
                      {saved > 0 && (
                ...
                <div className="mt-3 bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-xs text-green-400 leading-relaxed">
        ...
        <div className="mt-10 bg-[#13131F] border border-white/5 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              { step:"2", title:"Choose level",          desc:"Basic removes only comments and excess whitespace. Standard collapses all whitespace. Aggressive also removes redundant semicolons and optimises keyword spacing." },
              { step:"3", title:"Review the statistics", desc:"See original size vs output size, the percentage saved, and the estimated Gzip size. Open Pass Analysis to see exactly what was removed in each step." },
              { step:"4", title:"Copy or download",      desc:"Click Copy for the minified output, or Download to save as .min.js. Switch to Beautify to format any minified code for readability." },
            ].map(s => (
